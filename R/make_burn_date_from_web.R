...
final_output = "~/DATA/MODIS/USA_BURNDATE_UNCERTAINTY/"
...
for(j in 1:length(tiles)){
  ...
  newfilename1 <- paste0("BurnDateUncertainty", filename, ".tif")
  
  for (M in 1:length(hdfs)) {
    sds = get_subdatasets(hdfs[M])
    gdal_translate(sds[2], dst_dataset = newfilename1[M], overwrite=TRUE)
  }
  
  
  for(i in 2000:2017){
    tile_files = Sys.glob(paste("BurnDateUncertainty",i,"*.tif", sep = ""))
    print(tile_files)
    ...
    filename_for_the_tilefile = paste(output_directory, "AllYear_BDU_", tiles[j], "_", i, ".tif", sep="")
    
    writeRaster(three_whole_year_of_fire, filename_for_the_tilefile, format = "GTiff", overwrite=TRUE)
...
for(k in 2000:2017){
  ...
  whole_damn_country = raster::merge(raster(paste("AllYear_BDU_h08v04_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h08v05_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h09v04_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h09v05_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h10v04_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h10v05_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h10v06_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h11v04_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h11v05_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h12v04_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h12v05_",k,".tif", sep = "")),
                             raster(paste("AllYear_BDU_h13v04_",k,".tif", sep = "")))
  ...
  whole_damn_country_filename = paste0(final_output,"USA_burn_date_uncertainty_courtesy_of_Adam_Mahood", k, ".tif")
